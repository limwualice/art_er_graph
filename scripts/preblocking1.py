...
file_loc = file_location('clean_getty_csvs')
...
for i in range(1,n+1):
    print(i)
    df=pd.read_csv(f'{file_loc}/df{i}.csv', low_memory=False)
...
if data_dictionary: # Check if data_dictionary is not empty
    concatenated_file_location=file_location('concatenated_getty')
    ...
    for key, column_names_to_keep in data_dictionary.items():  # Iterate through the dictionary items
        ...
        concatenated_dfs[key].to_csv(f'{concatenated_file_location}/df_{key}.csv')
